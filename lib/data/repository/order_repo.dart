...
class OrderRepo extends GetxService {
  ...
  Future<Response> acceptOrder(int orderID, String deliveryTime) {
    return apiClient.postData(AppConstants.ACCEPT_ORDER_URI, {"_method": "put", 'token': getUserToken(), 'order_id': orderID, 'delivery_time': deliveryTime});
  }

  Future<Response> updateIsChecked(int orderId, int foodItemId, int isChecked){
    return apiClient.postData(AppConstants.Is_Checked_URI, {"_method": "put", 'token': getUserToken(), 'order_id': orderId, 'food_id': foodItemId, 'is_checked': isChecked});
  }
